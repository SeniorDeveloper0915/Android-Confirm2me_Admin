...
if (!isset($_SESSION['username'])) {
    ...
    $input['firstname']     = post('firstname');
    $input['lastname']      = post('lastname');
    $input['email']         = post('email');
    $input['password']      = post('password');
    $input['phonenumber']   = post('phonenumber');
    $input['pin']           = post('pin');
...
if ($_POST) {
    ...
    if ($statement->rowCount() > 0) {
        ...
        // if ($_FILES['image']['name'] == '') {
            $query      = 'INSERT INTO `users` SET username = ?, password=?, email=?, firstname = ?, lastname = ?, phonenumber = ?, PIN = ?';
            $parameters = array($input['username'], $input['password'], $input['email'], $input['firstname'], post('lastname'), post('phonenumber'), post('pin'));
